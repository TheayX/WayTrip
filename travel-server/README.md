...
- `UPLOAD_PATH` 决定文件实际落盘目录
- 数据库与前端仍使用 `/uploads/...` 访问路径，不直接保存物理磁盘路径
- 如果修改了 `UPLOAD_PATH`，旧文件不会自动迁移，需要手动迁移已有上传文件
- 建议生产环境使用绝对路径，避免不同启动目录导致相对路径解析结果变化
