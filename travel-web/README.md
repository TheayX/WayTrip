...
- 随心一选、穷游玩法、游客口碑、近期热看
- AI 客服聊天组件、会话态持久化、推荐解释与订单咨询入口

## AI 客服职责

- Web 端负责 AI 聊天窗口、建议问题、会话 `sessionId` 持久化和流式消息展示。
- 页面路由会向后端透传轻量 `scenarioHint`，用于辅助订单咨询、推荐解释、景点问答和行程规划等场景路由。
- 实际 AI 对话、Prompt 组装、RAG 检索、Function Calling 和记忆管理都由 `travel-server` 统一承接。
